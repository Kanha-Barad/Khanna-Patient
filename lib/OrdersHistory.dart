  ...
  Widget buildIconButton() {
    return Container(
      ...
      child: ElevatedButton.icon(
        onPressed: () {
          SRS_URL(data.bill_id.toString());
        },
        label: Text(
          'SRS',
          style:
              TextStyle(color: Color.fromARGB(255, 79, 54, 244), fontSize: 12),
        ),
        style: ButtonStyle(
          backgroundColor: MaterialStateProperty.all<Color>(Color.fromARGB(
              255, 223, 229, 235)), // Set the desired background color here
        ),
        icon: Icon(
          Icons.print_rounded,
...
SRS_URL(bill_id) async {
  var url = globals.SRS_Report + bill_id;
//NM live
  //"http://115.112.254.129/NM_SRV_GRP_MERGE_PDFS/" + reportCd + ".pdf"
  if (await launch(url)) {
    await launch(url);
